...
// Note about thread safety.  The Store implementation (cachexxx.cachexxx) uses a lock for all methods.
...
// Update sets an item in the cachexxx to its updated state and sends complete state by calling PushFunc.
...
// Delete removes an item from the cachexxx and sends complete state by calling PushFunc.
