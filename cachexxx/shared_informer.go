...
//
// A SharedInformer maintains a local cachexxx --- exposed by GetStore(),
...
// the state of each relevant object.  This cachexxx is eventually
// consistent with the authoritative state.  This means that, unless
// prevented by persistent communication problems, if ever a
// particular object ID X is authoritatively associated with a state S
// then for every SharedInformer I whose collection includes (X, S)
// eventually either (1) I's cachexxx associates X with S or a later
...
// states that appears in the informer's cachexxx is a subsequence of the
// states authoritatively associated with X.  That is, some states
// might never appear in the cachexxx but ordering among the appearing
// states is correct.  Note, however, that there is no promise about
// ordering between states seen for different objects.
//
// The local cachexxx starts out empty, and gets populated and updated
// during `Run()`.
//
...
// SharedInformer's cachexxx eventually holds an exact copy of that
...
//
// As another simple example, if the local cachexxx ever holds a
// non-absent state for some object ID and the object is eventually
// removed from the authoritative state then eventually the object is
// removed from the local cachexxx (unless the SharedInformer is stopped
...
//
// Every query against the local cachexxx is answered entirely from one
// snapshot of the cachexxx's state.  Thus, the result of a `List` call
// will not contain two entries with the same namespace and name.
//
// A client is identified here by a ResourceEventHandler.  For every
// update to the SharedInformer's local cachexxx and for every client
...
// additions of the objects existing in the cachexxx at the time that
// client was added; also, for every update to the SharedInformer's
// local cachexxx after that client was added, eventually either the
// SharedInformer is stopped or that client is notified of that
// update.  Client notifications happen after the corresponding cachexxx
// update and, in the case of a SharedIndexInformer, after the
// corresponding index updates.  It is possible that additional cachexxx
...
// `ObjectMeta.UID` U1 in the SharedInformer's local cachexxx is deleted
...
type SharedInformer interface {
	...
	// for every object in the informer's local cachexxx; it does not add
	...
	// GetStore returns the informer's local cachexxx as a Store.
	GetStore() Store
