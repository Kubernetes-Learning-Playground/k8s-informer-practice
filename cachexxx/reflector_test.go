...
// (In kubernetes 1.17, or when the watch cachexxx is enabled, the List will instead return the list that is no older than
	...
	lw := &testLW{
		...
		ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
			...
			case "0":
				return &v1.PodList{ListMeta: metav1.ListMeta{ResourceVersion: "10"}, Items: pods[0:4]}, nil
			case "10":
				// When watch cachexxx is disabled, if the exact ResourceVersion requested is not available, a "Expired" error is returned.
