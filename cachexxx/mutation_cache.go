...
// remains in the mutation cachexxx before it is removed.
//
// If includeAdds is true, objects in the mutation cachexxx will be returned even if they don't exist
// in the underlying store. This is only safe if your use of the cachexxx can handle mutation entries
// remaining in the cachexxx for up to ttl when mutations and deletes occur very closely in time.
...
func (c *mutationCache) ByIndex(name string, indexKey string) ([]interface{}, error) {
	...
	defer c.lock.Unlock()
	if c.indexer == nil {
		return nil, fmt.Errorf("no indexer has been provided to the mutation cachexxx")
	}
	...
	if c.includeAdds {
		...
		for _, key := range c.mutationCache.Keys() {
			...
			if err != nil {
				klog.V(4).Infof("Unable to calculate an index entry for mutation cachexxx entry %s: %v", key, err)
				continue
			}
...
// Mutation adds a change to the cachexxx that can be returned in GetByKey if it is newer than the backingCache
