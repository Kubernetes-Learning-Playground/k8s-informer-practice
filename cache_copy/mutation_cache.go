...
// remains in the mutation cache_copy before it is removed.
//
// If includeAdds is true, objects in the mutation cache_copy will be returned even if they don't exist
// in the underlying store. This is only safe if your use of the cache_copy can handle mutation entries
// remaining in the cache_copy for up to ttl when mutations and deletes occur very closely in time.
...
func (c *mutationCache) ByIndex(name string, indexKey string) ([]interface{}, error) {
	...
	defer c.lock.Unlock()
	if c.indexer == nil {
		return nil, fmt.Errorf("no indexer has been provided to the mutation cache_copy")
	}
	...
	if c.includeAdds {
		...
		for _, key := range c.mutationCache.Keys() {
			...
			if err != nil {
				klog.V(4).Infof("Unable to calculate an index entry for mutation cache_copy entry %s: %v", key, err)
				continue
			}
...
// Mutation adds a change to the cache_copy that can be returned in GetByKey if it is newer than the backingCache
