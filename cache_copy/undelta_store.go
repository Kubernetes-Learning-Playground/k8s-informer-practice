...
// Note about thread safety.  The Store implementation (cache_copy.cache_copy) uses a lock for all methods.
...
// Update sets an item in the cache_copy to its updated state and sends complete state by calling PushFunc.
...
// Delete removes an item from the cache_copy and sends complete state by calling PushFunc.
