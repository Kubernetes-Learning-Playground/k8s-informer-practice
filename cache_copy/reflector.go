...
type Reflector struct {
	...
	// it will turn off pagination to allow serving them from watch cache_copy.
...
var internalPackages = []string{"client-go/tools/cache_copy/"}
...
func (r *Reflector) list(stopCh <-chan struct{}) error {
	...
	go func() {
		...
		case r.paginatedResult:
			...
			// paging requests (i.e. watch cache_copy is probably disabled) and leave the default
			// pager size set.
		case options.ResourceVersion != "" && options.ResourceVersion != "0":
			// User didn't explicitly request pagination.
			//
			// With ResourceVersion != "", we have a possibility to list from watch cache_copy,
			// but we do that (for ResourceVersion != "0") only if Limit is unset.
			// To avoid thundering herd on etcd (e.g. on master upgrades), we explicitly
			// switch off pagination to force listing from watch cache_copy (if enabled).
			// With the existing semantic of RV (result is at least as fresh as provided RV),
			// this is correct and doesn't lead to going back in time.
			//
			// We also don't turn off pagination for ResourceVersion="0", since watch cache_copy
			// is ignoring Limit in that case anyway, and if watch cache_copy is not enabled
		...
		if isExpiredError(err) || isTooLargeResourceVersionError(err) {
			...
			// resource version it is listing at is expired or the cache_copy may not yet be synced to the provided
	...
	// which will return paginated result, even if watch cache_copy is enabled. However, in
	// that case, we still want to prefer sending requests to watch cache_copy if possible.
	//
	// Paginated result returned for request with ResourceVersion="0" mean that watch
	// cache_copy is disabled and there are a lot of objects of a given type. In such case,
	// there is no need to prefer listing from watch cache_copy.
...
func (r *Reflector) relistResourceVersion() string {
	...
	if r.isLastSyncResourceVersionUnavailable {
		// Since this reflector makes paginated list requests, and all paginated list requests skip the watch cache_copy
	...
	if r.lastSyncResourceVersion == "" {
		...
		// be served from the watch cache_copy if it is enabled.
		return "0"
	}
	return r.lastSyncResourceVersion
}
