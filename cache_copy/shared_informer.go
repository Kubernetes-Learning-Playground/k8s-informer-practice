...
//
// A SharedInformer maintains a local cache_copy --- exposed by GetStore(),
...
// the state of each relevant object.  This cache_copy is eventually
// consistent with the authoritative state.  This means that, unless
// prevented by persistent communication problems, if ever a
// particular object ID X is authoritatively associated with a state S
// then for every SharedInformer I whose collection includes (X, S)
// eventually either (1) I's cache_copy associates X with S or a later
...
// states that appears in the informer's cache_copy is a subsequence of the
// states authoritatively associated with X.  That is, some states
// might never appear in the cache_copy but ordering among the appearing
// states is correct.  Note, however, that there is no promise about
// ordering between states seen for different objects.
//
// The local cache_copy starts out empty, and gets populated and updated
// during `Run()`.
//
...
// SharedInformer's cache_copy eventually holds an exact copy of that
...
//
// As another simple example, if the local cache_copy ever holds a
// non-absent state for some object ID and the object is eventually
// removed from the authoritative state then eventually the object is
// removed from the local cache_copy (unless the SharedInformer is stopped
...
//
// Every query against the local cache_copy is answered entirely from one
// snapshot of the cache_copy's state.  Thus, the result of a `List` call
// will not contain two entries with the same namespace and name.
//
// A client is identified here by a ResourceEventHandler.  For every
// update to the SharedInformer's local cache_copy and for every client
...
// additions of the objects existing in the cache_copy at the time that
// client was added; also, for every update to the SharedInformer's
// local cache_copy after that client was added, eventually either the
// SharedInformer is stopped or that client is notified of that
// update.  Client notifications happen after the corresponding cache_copy
// update and, in the case of a SharedIndexInformer, after the
// corresponding index updates.  It is possible that additional cache_copy
...
// `ObjectMeta.UID` U1 in the SharedInformer's local cache_copy is deleted
...
type SharedInformer interface {
	...
	// for every object in the informer's local cache_copy; it does not add
	...
	// GetStore returns the informer's local cache_copy as a Store.
	GetStore() Store
